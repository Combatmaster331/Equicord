...
function Badges({ author }): JSX.Element | null {
    ...
    const [userEQBadges, setUserEQBadges] = useState<[string, string][]>([]);
    ...
    useEffect(() => {
        if (settings.store.showVencordDonorBadges) {
            fetch("https://gist.githubusercontent.com/Vendicated/51a3dd775f6920429ec6e9b735ca7f01/raw/badges.csv")
                ...
                .then((data: string) => {
                    const venBadges: string[][] = data.split("\n").map((s: string) => s.split(","));
                    const userBadges: [string, string][] = [];
                    for (let i: number = 0; i < venBadges.length; i++)
                        if (venBadges[i][0] === author.id)
                            userBadges.push([venBadges[i][1], venBadges[i][2]]);
        ...
        if (settings.store.showEquicordDonorBadges) {
            fetch("https://gist.githubusercontent.com/thororen1234/f51c43494e3a4a649f35f6a3bea3ca0b/raw/badges.csv")
                ...
                .then((data: string) => {
                    ...
                    const usrBadges: [string, string][] = [];
                    for (let i: number = 0; i < equiBadges.length; i++)
                        if (equiBadges[i][0] === author.id)
                            usrBadges.push([equiBadges[i][1], equiBadges[i][2]]);
                    setUserEQBadges(usrBadges);
                })
                .catch(e => { console.error(e); });
        }
